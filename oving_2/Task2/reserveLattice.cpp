...
void Lattice::findBiggestCluster(int potentiallyBigger){
  ...
  //CHECK IF INCORRECT BIGGEST CLUSTER IS CHOSEN
  /*
  for (int i = 0; i < clusters.size(); i++){
    if (clusters[i] < clusters[biggestCluster]){
      perror("Incorrect biggest cluster set.");
    }
  }*/
}
...
void Lattice::weightedAvgClusterSize(){
  ...
  if(p_inf >= 1.0){
    ...
    w_clusterSize = ((double)average_s-pow(N_d*N_d*p_inf,2))/(N_d*N_d*(1.0-p_inf));
  }
